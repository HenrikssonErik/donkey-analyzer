 ...
 *
 */
#ifndef __HISTOGRAMROOT_HPP__
#define __HISTOGRAMROOT_HPP__
...
class HistogramRoot {
public:
    static HistogramRoot* get_instance();
    ~HistogramRoot();
...
private:
    static HistogramRoot* histogram;
    ...
    unsigned long sketch[SKETCH_SIZE_ROOT];
    double hash[SKETCH_SIZE_ROOT];
    double powerful;
#ifdef MEMORY   
    /* PREGEN is a compilation constant defined using -D flag.
     * It is the number of random variables we sampled ahead of time. */
    double gamma_param[PREGEN][SKETCH_SIZE_ROOT];
    double r_beta_param[PREGEN][SKETCH_SIZE_ROOT];
    double power_r[PREGEN][SKETCH_SIZE_ROOT];
#endif
